...
public class Data {
    ...
    public static double getSigma(H1F hires, double mean) {
        ...
        double count = 0;
        for (int i=0; i < hires.getAxis().getNBins(); ++i) {
            double val = hires.getAxis().getBinCenter(i)*hires.getBinContent(i)
                    - mean;
            summ  += val*val;
            count += hires.getBinContent(i);
        }
     ...
     */
    public static DataGroup[] createResDataGroups(int type, int ln, int zn,
            int r) {
