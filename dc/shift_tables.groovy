...
public class shift_tables {
	...
	public static void main(String[] args) {
    	...
    	// file list of the shifted geometry
		if (args.length < 2) {
			// exits program if input directory not specified 
    	...
    	if (args.length < 4) {
			...
			theta_bins = Integer.parseInt(args[2]);
			phi_bins = Integer.parseInt(args[3]);
		}
		...
		if (args.length < 6) {
			...
			for (int i=0; i<theta_bins; i++) {
				theta_limits[i+1] = Integer.parseInt(args[4+i]);
			}
			for (int i=0; i<phi_bins; i++) {
				phi_limits[i+1] = Integer.parseInt(args[4+theta_bins+i]);
			}
		}
		...
		H1F[][][] shift_residuals = new H1F[theta_bins][phi_bins][36]; 
		for (int layer = 0; layer < 36; layer++) {
			for (int i = 0; i<theta_bins; i++) {
				for (int j = 0; j<phi_bins; j++) {
					...
					shift_residuals[i][j][layer] = new H1F(" L"+Integer.toString(layer+1),
						n_bins,min_bin,max_bin); 
				}
			}
		}
		...
		// same process as the above for loop only this time for the second directory
		for (int current_file; current_file<n_files; current_file++) {
			println(); println(); println("Opening file "+Integer.toString(current_file+1)
								...
								for (int i = 0; i<theta_bins; i++) {
									for (int j = 0; j<phi_bins; j++) {
										...
										if ((theta<theta_limits[i+1]&&theta>theta_limits[i])&&
											(rotated_phi<phi_limits[j+1]&&
											rotated_phi>phi_limits[j])) {
											shift_residuals[i][j][(superlayer-1)*
											6+(layer)-1].fill(residual)
										}
									}
								}
		...
		// the point is to print out vectors for the means of the residuals that can be copy and 
		// pasted into a root MINUIT program for each of the shift values
		...
		for (int i=0; i<theta_bins; i++) {
			for (int j=0; j<phi_bins; j++) {
				...
				double[] differences = new double[36];

				print("{");
				for (int layer = 0; layer < 36; layer++) {
					// calculate statistics for each of the layers
					double shift_mean = shift_residuals[i][j][layer].getMean();
					double shift_std = shift_residuals[i][j][layer].getRMS();
					double shift_counts = shift_residuals[i][j][layer].getEntries();
